...
Вызовайте в чате Cursor соответствующего **агента** (по имени из frontmatter). В сообщении по возможности укажите `run=<путь-к-папке-run>` и для планировщика/разработчика — `mode=full` или `mode=fast`, если режим неочевиден. Активный прогон по умолчанию — первая строка файла `.cursor/tasks/runs/LATEST`.
